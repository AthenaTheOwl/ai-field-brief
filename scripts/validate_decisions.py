"""Validate decision records (DEC-*.md) in ai-field-brief.
...
REMOTE_URL = os.environ.get("AI_FIELD_BRIEF_SCHEMA_URL_BASE", DEFAULT_REMOTE_URL)
...
def load_remote_schema() -> dict[str, Any] | None:
    try:
        req = urllib.request.Request(
            REMOTE_URL, headers={"User-Agent": "ai-field-brief/validate_decisions"}
        )
...
def main() -> int:
    ...
    for dec_path in decisions:
        ...
        # Normalize date: PyYAML parses a YYYY-MM-DD scalar into a
        # datetime.date, but the JSON Schema validator expects a
        # string with format: date. Coerce to ISO string before validating.
        if "date" in data:
            value = data["date"]
            try:
