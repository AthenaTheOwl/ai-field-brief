"""Validate role contracts (.agents/roles/<id>/role.yaml) in ai-field-brief.

Walks every `.agents/roles/*/role.yaml`, parses the YAML, and validates
the parsed object against the cross-repo `role.schema.json` sourced from
athena-site. The schema is fetched at run time from
`https://raw.githubusercontent.com/AthenaTheOwl/athena-site/main/ops/schemas/role.schema.json`,
with a local cache fallback at `ops/schemas-cache/role.schema.json`
so the script runs offline.
...
def load_remote_schema() -> dict[str, Any] | None:
    try:
        req = urllib.request.Request(
            REMOTE_URL, headers={"User-Agent": "ai-field-brief/validate_roles"}
        )
...
def discover_role_files() -> list[Path]:
...
def main() -> int:
    ...
    for role_path in role_files:
        ...
        if isinstance(rid, str):
            parent_slug = role_path.parent.name
            if rid != parent_slug:
                violations.append(
                    f"{rel}: id `{rid}` does not match parent directory name "
                    f"`{parent_slug}`"
                )
            if rid in seen_ids:
                violations.append(
                    f"{rel}: duplicate role id `{rid}` (also defined in "
                    f"{seen_ids[rid].relative_to(ROOT).as_posix()})"
                )
            seen_ids[rid] = role_path
    ...
    print(f"validate_roles OK ({len(role_files)} role(s))")
    return 0
