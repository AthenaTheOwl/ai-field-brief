...
Spec 0010 installs the Cognitive Delivery Control Plane (CDCP) governance
scaffold in ai-field-brief. Specs already name the what. This spec adds
the parallel records that name the why (decisions), the what-we-reuse
(skills), and the what-we-learned (dreams), plus the executable gates
that fail builds when any of those records drift out of shape.
...
title, scope (1-2 sentences), and proof refs.

Acceptance:
- `ops/RELEASE_LEDGER.md` carries one entry per released commit.
- The current backfill records the nine pre-CDCP commits from f126a87
  through 992f3f2.
...
Acceptance:
- `.agents/skills/run-weekly-brief/SKILL.md` graduates the existing
  `playbook/run-weekly-brief.md` pattern.
- The SKILL.md front-matter matches the cross-repo
  `skill.schema.json` shape.
...
WHEN any of `spec_check`, `voice_lint`, `validate_schemas`,
`validate_registry`, or `validate_decisions` fails, THE SYSTEM SHALL
fail the CI job and block the PR from merging.

Acceptance:
- `.github/workflows/ci.yml` runs all five python gates in the
  `gates` job.
...
Acceptance:
- `scripts/validate_decisions.py` fetches the schema by URL with a
